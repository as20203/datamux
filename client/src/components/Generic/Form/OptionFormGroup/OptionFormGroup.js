...
const OptionFormGroup = (props)=>{
    const options = props.options.map((option,index)=><option customer-key={option.customer || ''} value={option} data-key={option.key || index} key={index}>{option.value || option}</option>)
    return(
