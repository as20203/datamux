...
const NetworkServer= ()=>{
    ...
    const retrieveUserSession = async () => {
        ...
        const { data: {result: { authToken } } } = await axios.post('/api/proxy',credentials,{params: {url:  'https://ns.talkpool.com/login' } }, );
    ...
    const onSubmit = async(e) =>{
        ...
        await Promise.all(
            csvData.map(async record=>{
                try{
                    if (isMounted.current) await axios.post('/api/proxy',record, { headers: {'X-AUTH-TOKEN': `${authToken}` }, params: {url:  'https://ns.talkpool.com/internal/devices' }   })    
                }catch({response}){
                    console.log(response);
                    errorDevices.push({Deviceeui:record.deviceEui,Error: response.data.error.detailedErrorMessage})
                }
            })
        )
    ...
    const updateData = (result) => {
        ...
        console.log(data);
        const validCSV = data.every(item=> item.hasOwnProperty('deviceType') && item.hasOwnProperty('deviceEui')
        && item.hasOwnProperty('appEui') && item.hasOwnProperty('appKey') 
        );
        if(!validCSV){
